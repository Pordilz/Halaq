...
export default function Login() {
  ...
  if (view === 'forgot' || view === 'forgot-success') {
     ...
     return (
        <div className="bg-surface" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 1.5rem', width: '100%', maxWidth: '28rem', margin: '0 auto', transition: 'opacity 250ms ease', opacity: isAnimating ? 0 : 1 }}>
            <div className="shadow-standard" style={{ width: '100%', backgroundColor: 'white', borderRadius: '1.5rem', padding: '2rem' }}>
              {isSuccess ? (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                  <MaterialIcon name="mail_outline" size={64} className="text-primary" style={{ marginBottom: '1rem' }} />
                  <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--color-on-surface)', marginBottom: '0.5rem' }}>Check your email</h1>
                  <p style={{ fontSize: '0.875rem', color: 'var(--color-on-surface-variant)', lineHeight: 1.6, marginBottom: '1.5rem' }}>We've sent a reset link to <strong>{email}</strong>. It expires in 15 minutes.</p>
                  <button onClick={() => transitionView('login')} className="text-primary" style={{ fontSize: '0.875rem', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer' }}>Back to sign in</button>
                </div>
              ) : (
                <form onSubmit={handleResetSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  <button type="button" onClick={() => transitionView('login')} className="text-primary" style={{ fontSize: '0.875rem', display: 'flex', alignItems: 'center', marginBottom: '0.5rem', width: 'max-content', background: 'none', border: 'none', cursor: 'pointer' }}>← Back to sign in</button>
                  <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--color-on-surface)' }}>Reset your password</h1>
                  <p style={{ fontSize: '0.875rem', color: 'var(--color-on-surface-variant)', lineHeight: 1.6, marginBottom: '0.5rem' }}>Enter your email and we'll send you a reset link.</p>
                  ...
                  <input 
                    ...
                    required 
                  />
                  <button type="submit" className="btn btn-primary" style={{ width: '100%', height: '3.5rem', marginTop: '0.5rem', borderRadius: '1rem', fontSize: '1.125rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', background: 'linear-gradient(135deg, #1a6b47 0%, #22875a 100%)', color: 'white', fontWeight: 700, border: 'none', cursor: 'pointer', transition: 'transform 0.15s' }} disabled={loading}>
                    {loading ? <MaterialIcon name="refresh" className="spinner" size={24} /> : 'Send Reset Link →'}
                  </button>
                </form>
              )}
            </div>
  ...
  // ==== Login View ====
  const formContent = (
    <div style={{ transition: 'opacity 250ms ease', opacity: isAnimating ? 0 : 1 }}>
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {error && (
          <div className="bg-error-container text-on-error-container" style={{ padding: '1rem', borderRadius: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem', fontSize: '0.875rem', fontWeight: 600 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', width: '100%', position: 'relative' }}>
          <label style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--color-on-surface-variant)', marginLeft: '0.25rem' }}>Password</label>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '0.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <input 
              type="checkbox" 
              style={{ width: '1.25rem', height: '1.25rem', accentColor: 'var(--color-primary)', borderRadius: '0.25rem' }}
              checked={rememberMe}
              onChange={(e) => setRememberMe(e.target.checked)}
            />
            <label style={{ fontSize: '0.875rem', color: 'var(--color-on-surface-variant)' }}>Keep me signed in</label>
          </div>
          <button type="button" onClick={() => transitionView('forgot')} className="text-primary" style={{ fontSize: '0.875rem', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer' }}>Forgot your password?</button>
        </div>

        <button type="submit" className="btn btn-primary" style={{ width: '100%', height: '3.5rem', marginTop: '1rem', borderRadius: '1rem', fontSize: '1.125rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', background: 'linear-gradient(135deg, #1a6b47 0%, #22875a 100%)', color: 'white', fontWeight: 700, border: 'none', cursor: 'pointer', transition: 'transform 0.15s' }} disabled={loading}>
          {loading ? <MaterialIcon name="refresh" className="spinner" size={24} /> : 'Sign In'}
        </button>
      </form>
      ...
      <div style={{ fontSize: '0.875rem', marginTop: '2rem', textAlign: 'center', color: 'var(--color-on-surface-variant)' }}>
        Don't have an account? <Link to="/signup" style={{ color: 'var(--color-primary)', fontWeight: 800, textDecoration: 'none' }}>Create one →</Link>
      </div>
    </div>
  );
    ...
    <div className="bg-surface" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="desktop-auth-container animate-scale-in" style={{ display: 'none', width: '100%', maxWidth: '960px', backgroundColor: 'white', borderRadius: '1.5rem', boxShadow: 'var(--shadow-floating)', overflow: 'hidden', animation: 'scaleIn 0.5s ease 0.1s both', zIndex: 10 }}>
        ...
        <div style={{ width: '40%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '3.5rem', position: 'relative', background: 'linear-gradient(160deg, #1a6b47 0%, #22875a 100%)', minHeight: '560px' }}>
           ...
           <div style={{ position: 'relative', zIndex: 10 }}>
             ...
             <h2 style={{ fontSize: '2rem', fontWeight: 800, color: 'rgba(255,255,255,0.95)', lineHeight: 1.25 }}>Welcome back.</h2>
             <p style={{ fontSize: '0.875rem', color: 'rgba(255,255,255,0.75)', marginTop: '0.75rem' }}>Your halal portfolio is waiting.</p>
           </div>

           <div style={{ position: 'relative', zIndex: 10, marginTop: 'auto' }}>
              <div style={{ backgroundColor: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(12px)', borderRadius: '1rem', padding: '1rem', width: '100%' }}>
                 <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}><MaterialIcon name="bookmarks" fill size={20} className="text-white" /><span style={{ fontSize: '0.875rem', color: 'white', fontWeight: 700 }}>Watchlist Preview</span></div>
                 <div style={{ fontSize: '0.875rem', color: 'rgba(255,255,255,0.9)' }}>12 stocks · 10 Compliant · 2 Review</div>
              </div>
           </div>
        </div>
        ...
        <div style={{ width: '60%', backgroundColor: 'var(--color-surface-lowest)', padding: '3.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
           <div style={{ transition: 'opacity 250ms ease', opacity: isAnimating ? 0 : 1 }}>
             <h1 style={{ fontSize: '2.5rem', fontWeight: 800, letterSpacing: '-0.025em', lineHeight: 1.1, color: 'var(--color-on-surface)', marginBottom: '0.5rem' }}>Welcome Back<br/>to <span className="text-primary">Halaq.</span></h1>
             <p style={{ fontSize: '1rem', color: 'var(--color-on-surface-variant)', marginBottom: '2rem' }}>Continue your halal investing journey.</p>
           </div>
           {formContent}
        </div>
      </div>
      ...
      <div className="mobile-auth-container animate-entrance delay-100" style={{ width: '100%', flex: 1, display: 'flex', flexDirection: 'column' }}>
        <header className="glass-panel" style={{ height: '4rem', display: 'flex', alignItems: 'center', padding: '0 1rem', width: '100%', borderBottom: '1px solid var(--color-surface-container-low)', position: 'relative' }}>
          ...
          <span style={{ fontWeight: 700, fontSize: '1.25rem', color: 'var(--color-primary)', letterSpacing: '-0.025em', marginLeft: '0.5rem' }}>Sign In</span>
        </header>

        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4rem 1.5rem 3rem 1.5rem', width: '100%', maxWidth: '28rem', margin: '0 auto', transition: 'opacity 250ms ease', opacity: isAnimating ? 0 : 1 }}>
          <div className="animate-entrance delay-100" style={{ width: '100%' }}>
            <h1 style={{ fontSize: '2.5rem', fontWeight: 800, letterSpacing: '-0.025em', lineHeight: 1.15, color: 'var(--color-on-surface)', marginBottom: '0.75rem' }}>Welcome Back<br/><span className="text-primary">to Halaq.</span></h1>
            <p style={{ fontSize: '1.125rem', fontWeight: 500, color: 'var(--color-on-surface-variant)', opacity: 0.8, maxWidth: '280px', lineHeight: 1.6 }}>Continue your halal investing journey.</p>
          </div>
